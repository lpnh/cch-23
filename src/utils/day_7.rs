...
pub fn bake(recipe_json: &str) -> Result<(i64, HashMap<String, i64>), &'static str> {
    let recipe_and_pantry: RecipeAndPantry =
        from_str(recipe_json).map_err(|_| "Failed to parse JSON")?;

    let RecipeAndPantry { recipe, pantry } = recipe_and_pantry;

    let cookies =
        min_possible_cookies(&recipe, &pantry).ok_or("Mismatch in recipe and pantry items")?;
...
fn min_possible_cookies(
    recipe: &HashMap<String, i64>,
    pantry: &HashMap<String, i64>,
) -> Option<i64> {
    recipe
        .iter()
        .filter_map(|(ingredient, &amount_needed)| {
            pantry
                .get(ingredient)
                .and_then(|&amount_available| amount_available.checked_div(amount_needed))
        })
        .min()
}

fn pantry_after_baking(
    recipe: &HashMap<String, i64>,
    pantry: &HashMap<String, i64>,
    cookies: i64,
